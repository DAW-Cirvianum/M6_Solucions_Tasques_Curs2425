...
let intentsRestants = 3; // Afegit tipus `number`
// Variable per indicar si l'usuari ha superat el captcha
let captchaSuperat = false; // Afegit tipus `boolean`
...
while (intentsRestants > 0 && !captchaSuperat) {
    ...
    let respostaCorrecta; // Afegit tipus `number`
    if (operacio === '+') {
    ...
    const respostaUsuariInput = prompt(`Intent ${4 - intentsRestants}: Quant és ${num1} ${operacio} ${num2}?`); // Afegit tipus `string | null` per gestionar el cas que `prompt` retorni `null`
    // Convertir la resposta de l'usuari a un nombre enter
    const respostaUsuari = respostaUsuariInput !== null ? parseInt(respostaUsuariInput) : NaN;
